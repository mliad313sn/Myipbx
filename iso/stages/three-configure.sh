...
configure_services() {
    ...
    # it generates its own before the console can serve. The unit is ordered
...
configure_boot_message() {
  ...
  This appliance generated its own certificate the first time it started, so
  no two appliances share one. Your browser will warn about it. The
  fingerprint to compare against is printed on this screen beside the
...
audit_image() {
    ...
    # Nor may a certificate or its private key. Every appliance booted from
    # this image would otherwise hold the same key, and anybody who downloaded
    # the image could read the console traffic of every site running it. That
    ...
    log_info "the image audit passed: no allocation service, and neither a credential nor a private key shipped"
}
